...
export async function getAdminStats() {
  const db = await getDb();
  if (!db) return { totalStudios: 0, totalClients: 0, licenseStats: [] };
  try {
    const [sr] = await (db as any).$client.query("SELECT COUNT(*) as total FROM users WHERE role != 'admin'");
    const [cr] = await (db as any).$client.query("SELECT COUNT(*) as total FROM clients");
    const [lr] = await (db as any).$client.query("SELECT status, planType, COUNT(*) as count FROM licenses GROUP BY status, planType");
    return {
      totalStudios: (sr as any)[0]?.total || 0,
      totalClients: (cr as any)[0]?.total || 0,
      licenseStats: lr as any[],
    };
...
export async function getArchivesNumerisees(userId: number) {
  ...
  const [rows] = await (db as any).$client.query(
    'SELECT id, user_id, nom, prenom, date_numerisation as dateNumerisation, type_document as typeDocument, praticien, periode, notes, photos, created_at as createdAt FROM archives_numerisees WHERE user_id = ? ORDER BY created_at DESC',
    [userId]
  );
...
export async function deleteArchiveNumerisee(userId: number, id: number) {
  const db = await getDb();
  if (!db) throw new Error('DB');
  await (db as any).$client.query('DELETE FROM archives_numerisees WHERE id = ? AND user_id = ?', [id, userId]);
  return { success: true };
}
