...
export default function Layout({ children }: { children: React.ReactNode }) {
  ...
  const activeIdentityName = isEmployeLoggedIn && employe
    ? [employe.prenom, employe.nom].filter(Boolean).join(' ')
    : state.salonInfo?.nom || 'Salon';
  const activeIdentityLabel = isEmployeLoggedIn ? 'Salarié connecté' : 'Session salon';
  ...
  const visibleNavSections = isEmployeLoggedIn
    ? [{ sectionKey: null, items: [{ path: '/clients', icon: Users, labelKey: 'nav.clients' }] }]
    : NAV_SECTIONS;

  const handleLogout = () => {
    if (isEmployeLoggedIn) {
      logoutEmploye();
      navigate('/pin');
      return;
    }
